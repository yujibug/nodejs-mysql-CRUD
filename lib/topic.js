...
exports.home = (request, response) => {
  function home(sortBy) {
    ...
    const PAGECONTENT = 3; //한 page안에 보여줄 title 갯수
...
exports.page = (request, response) => {
  function page(sortBy) {
    ...
    db.query(`SELECT COUNT(*) FROM topic`, (error, count) => {
      ...
      db.query(`SELECT * FROM topic ` + sortBy, (error, topics) => {
        if (error) {
          throw error;
        }
        db.query(
          `SELECT * FROM topic LEFT JOIN author ON topic.author_id = author.id WHERE topic.id=${queryData} ` +
            sortBy +
            'LIMIT ' +
            PAGECONTENT +
            ' OFFSET ' +
